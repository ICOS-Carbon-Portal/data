...
class ZipValidatorTests extends AnyFunSuite with BeforeAndAfterAll:
	...
	val zipSink = assertFormat(using system.dispatcher)
	...
	testFile("Valid zip file", "blablaText.txt.zip", Valid)
	testFile("Empty zip file", "empty.zip", EmptyZip)
	testFile("Invalid zip file", "fluxnet_hh.csv", Invalid)
	testFile("Spanned zip file", "spanned.zip", SpannedZip)
