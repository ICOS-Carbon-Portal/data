...
import {type KnownDataObject, type ExtendedDobjInfo, type LabelLookup} from "../models/State";
	...
	useEffect(() => {
		...
		for (const readyObjectId of readyObjectIds) {
			if (!infoCache[readyObjectId]) {
				const spec = objectsTable.find(dataObject => dataObject.dobj === readyObjectId)?.spec;

				const stationId = extendedDobjInfo.find(edobj => edobj.dobj === readyObjectId)?.stationId;
