...
export default class LinearCache {
	...
	constructor(
		...
		private readonly startLimit: number,
	...
	fetch(offset: number, limit: number) {
		const fromCache = () =>
			this.cache.slice(offset - this.offset, offset - this.offset + limit);

		const useCache =
			offset >= this.offset
			&& (this.isDataEndReached || offset + limit <= this.offset + this.length);
		...
		return this.fetcher(offset, fetchingLimit + 1).then((arr) => {
			...
			if (this.length === 0 || offset === this.offset + this.length) {
				...
				offset + fetchingLimit > this.offset
				&& offset + fetchingLimit < this.offset + this.length
			) {
				// --≡≡==
