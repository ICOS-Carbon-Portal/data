...
export class FilterNumbers {
	...
	static deserialize(jsonFilterNumber: FilterNumberSerialized[]) {
		...
		const restoredFilterNumbers = new FilterNumbers(
			jsonFilterNumber.map((jfn) => new FilterNumber(jfn.cat, jfn.txt)),
		);
	...
	withFilter(filter: FilterNumber) {
		return this.list.length > 0 ?
				new FilterNumbers(
					this.list.map((nf) => (nf.category === filter.category ? filter : nf)),
				)
	...
	get validFilters() {
		return this.list.filter(
			(nf) => nf.isValid && nf.vals.length > 0 && nf.vals.length === nf.cmp.length,
		);
	}
...
const validate = (category: NumberFilterCategories, value: string): NumberFilterValidation => {
	...
	if (list.length) {
		return list.reduce<NumberFilterValidation>(
			(acc, curr) => {
