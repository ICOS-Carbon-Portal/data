...
export default class PreviewLookup {
	constructor(
		...
		public readonly varInfo: VarInfo,
	) {}
...
function getTable(specTable: CompositeSpecTable, labelLookup: LabelLookup): Table {
	...
	function latLonPresent(cols: Array<{ varTitle: string; valType: string }>): boolean {
		return (
			cols.some((col) => col.valType === config.mapGraph.latValueType)
	...
	Object.entries(specFormats).forEach(([spec, format]) => {
		...
		if (format === config.netCdf.format) table[spec] = { type: "NETCDF" };
		else if (config.imageMultiZipFormats.includes(format)) table[spec] = { type: "PHENOCAM" };
