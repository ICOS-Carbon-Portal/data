...
function getTable(specTable: CompositeSpecTable, labelLookup: LabelLookup): Table {
	...
	for (const {spec, format} of specTable.basics.rows) {
		if (typeof spec === "string" && typeof format === "string") {
			specFormats[spec] = format;
		}
	}

	for (const {spec, varTitle, valType} of specTable.columnMeta.rows) {
	...
	for (const [spec, format] of Object.entries(specFormats)) {
