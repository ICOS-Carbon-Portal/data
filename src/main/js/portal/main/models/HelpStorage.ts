...
export default class HelpStorage {
	...
	static deserialize(json: HelpStorage) {
		const helpItems = Object.values(json.helpItems).flatMap((js) =>
			js ? [HelpItem.clone(js)] : [],
		);
...
const initItems: HelpItem[] = [
	...
	new HelpItem(
		"station",
		"If applicable, the research station that produced the original data for this data object. "
	...
	new HelpItem(
		...
		[
			...
			},
			{
				label: "Associated",
				comment:
					'Associated with ICOS, but the data does not have the ICOS "quality stamp" (even if the data belongs to ICOS project)',
			},
			{
				label: "Other",
				comment:
					"Research stations unrelated to ICOS (or whose data present here are not ICOS data)",
			},
		],
		undefined,
	),

	new HelpItem("ecosystem", {
	...
	new HelpItem(
		"submitter",
		"Organization credited for submission of the data object. "
			+ "Acquisition and production are credited independently of submission.",
	),

	new HelpItem(
		"type",
		"Kind of data object. Encompasses most of characteristics related to data content, "
	...
	new HelpItem(
		"level",
		envri
			+ " distinguishes 4 levels of data in terms of how processed they are"
			+ (envri === "ICOS" ? " (ranging from raw data to modelling results)" : "")
			+ ":",
		undefined,
		envri === "SITES" ?
			[
				{
					label: 0 as Int,
					comment:
						"Unprocessed instrument or digtalized data at full time resolution with all available supplemental information to be used in"
				...
				},
				{
					label: 1 as Int,
					comment:
						"Calibrated, quality filtered internal working data in physical units. In case L0 data are already calibrated, L0 and L1 are"
				...
				},
				{
					label: 2 as Int,
					comment:
						"Quality checked SITES data product. It is calibrated, quality filtered data in physical units, a aggregated to appropariate,"
						+ ` and within SITES community agreed, spatial and temporal output units and resolution. Distributed by ${projectStr}.`,
				},
				{
					label: 3 as Int,
					comment:
						"Environmental variables or products produced by SITES or anywere in the scientific community. The product is derived from"
		...
		:	[
				{
					label: 0 as Int,
					comment:
						"Data in physical units either directly provided by the instruments or converted from engineer units (e.g. mV, mA, Ω) to"
				...
				},
				{
					label: 2 as Int,
					comment:
						`The final quality checked ICOS RI data set, published by the CFs, to be distributed through ${projectStr}. This level is`
						+ " the ICOS-data product and free available for users.",
				},
				{
					label: 3 as Int,
					comment:
						"All kinds of elaborated products by scientific communities that rely on ICOS data products are called Level 3 data.",
				},
			],
		[
			{
				txt: "Read more about data levels",
				url: "https://www.icos-cp.eu/data-services/data-collection/data-levels-quality",
			},
		],
	),
	...
	new HelpItem(
		"keywordFilter",
		"Keywords associated with the data objects. Note that, unlike other filter selections, keyword selection will find "
			+ 'data objects containing ALL keywords (boolean "AND"), not those containing ANY keyword (boolean "OR").',
	),

	new HelpItem(
		"format",
		"Technical file format, indicating which software module is needed to read the data",
	),

	new HelpItem(
		"variable",
		"Variable used in the actual data objects, such as column name in a tabular time-series dataset. "
	...
	new HelpItem(
		"valType",
		"A specific kind of physical quantity used in a certain scientific field. "
	...
	new HelpItem("preview", 'How "Preview" and "Add to cart" buttons work:', undefined, [
		{
			...
			comment:
				"normally available for single-table data objects of levels 1 and 2 and for NetCDF data objects (level 3)",
		},
		{
			label: "data cart availability",
			comment:
				"available for logged-in users; allows batch-downloading of multiple data objects",
		},
		{
			label: "how to preview",
			comment:
				"select the data object(s) by ticking checkboxes, click the preview button (if active)",
		},
		{
			label: "multi-object preview",
			comment:
				"tabular objects of the same data type can be previewed simultaneously; if coming from the same station and from disjoint time intervals, "
		...
		[
			{
				label: "full CSV",
				comment:
					"if column information is excluded from the URL, all the columns described in the metadata will be downloaded",
			},
			{
				label: "multi-preview",
				comment:
					"in the case of multiple-dataset preview, the URL will only provide data from one of the datasets",
			},
			{
				label: "licencing",
				comment:
					"you can only download the tabular data as CSV if you are logged in with the portal and have accepted the license agreement in your user profile",
			},
			{
				label: "disclaimer",
				comment:
					`CSV download is an extra service offered by ${projectStr} on a best-effort basis; `
		...
		[
			{
				...
				comment:
					`Paste in ${envri} Data Portal's PID of a data object, either complete with prefix, or just the suffix (24 characters).`
					+ "Full URLs including the PID web resolver gateway (hdl.handle.net) are permitted as well.",
			},
			{
				label: "Collection PID",
				comment:
					`Paste in ${envri} Data Portal's PID of a collection, either complete with prefix, or just the suffix (24 characters).`
					+ "Full URLs including the PID web resolver gateway (hdl.handle.net) are permitted as well.",
			},
			{
				label: "File name",
				comment:
					"Paste in a complete filename, with extension. This filter does not search for partial filenames.",
			},
		],
	),
];
