...
export default class InitMap {
	...
	constructor(props: Props) {
		...
		this.stationFilterControl = new StationFilterControl({
			element: document.querySelector("#stationFilterCtrl") ?? undefined,
		...
		this.layerControl = new LayerControl({
			element: document.querySelector("#layerCtrl") ?? undefined,
			selectedBaseMap,
		...
		const minWidth = 600;
		const width = document.querySelectorAll("body")[0].getBoundingClientRect().width;
		if (width < minWidth) {
			return;
		}
	...
	private createProjectionControl(persistedMapProps: PersistedMapPropsExtended, updateMapSelectedSRID: UpdateMapSelectedSRID) {
		return new ProjectionControl({
			element: document.querySelector("#projSwitchCtrl") ?? undefined,
	...
	updateLayerCtrl(self: LayerControl): () => void {
		return () => {
			...
			if (toggles.length > 0) {
				...
				for (const toggleLayer of toggles
					.filter(toggleLayer => toggleLayer.get("id") === countryBordersId)) {
					addToggleLayer(toggleLayer as VectorLayer<VectorSource>);
				}
