...
export default class InitMap {
	...
	constructor(props: Props) {
		...
		this.getStationPosLookup = () =>
			this.allStations.reduce<StationPosLookup>((acc, st) => {
		...
		if (Object.keys(olMapSettings.sridsInMap).length > 1)
			controls.push(
				this.createProjectionControl(persistedMapProps, props.updateMapSelectedSRID),
			);
		...
		getESRICopyRight(esriBaseMapNames).then((attributions) => {
			this.olWrapper.attributionUpdater = new Copyright(
				attributions,
				projection,
				"baseMapAttribution",
				minWidth,
	...
	private async fetchCountriesTopo() {
		...
		this.olWrapper.addGeoJson(
			countriesTopoBM,
			"EPSG:4326",
			this.olWrapper.projection,
			this.olWrapper.viewParams.extent,
		);
	...
	private createProjectionControl(
		persistedMapProps: PersistedMapPropsExtended,
		updateMapSelectedSRID: UpdateMapSelectedSRID,
	) {
	...
	private updatePoints(mapProps: MapProps) {
		...
		const excludedStations = createPointData(excludedUris, this.stationPosLookup, {
			[isIncludedStation]: false,
		});
		const includedStations = createPointData(this.selectedStations, this.stationPosLookup, {
			[isIncludedStation]: true,
		});
	...
	getLayerWrapper({
		...
		zIndex,
		interactive,
	}: Omit<LayerWrapperArgs, "visible">): LayerWrapper {
		const visible =
			layerType === "toggle" ?
	...
	updateLayerCtrl(self: LayerControl): () => void {
		return () => {
			...
			if (toggles.length) {
				const addToggleLayer = (
					toggleLayer: VectorLayer<VectorSource<Feature<Geometry>>>,
				) => {
					...
					toggle.addEventListener("change", () =>
						self.toggleLayers(toggleLayer.get("id"), toggle.checked),
					);
					row.appendChild(toggle);

					if (legendItem) {
						const legendItemContainer = document.createElement("span");
						legendItemContainer.setAttribute(
							"style",
							"display:table-cell; width:21px; text-align:center;",
						);
				...
				toggles
					...
					.forEach((toggleLayer) =>
						addToggleLayer(toggleLayer as VectorLayer<VectorSource<Feature<Geometry>>>),
					);
				toggles
					.filter((toggleLayer) => toggleLayer.get("id") !== countryBordersId)
					.forEach((toggleLayer) =>
		...
		select.on("select", (e) => {
			...
			if (numberOfFeatures) {
				...
				popupOverlay.setPosition(
					feature.getGeometry()?.getCoordinates() ?? e.mapBrowserEvent.coordinate,
				);
			} else {
...
function createPointData(
	stations: Value[],
	stationPosLookup: StationPosLookup,
	additionalAttributes: PointData["attributes"] = {},
): PointData[] {
