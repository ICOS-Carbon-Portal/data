...
export default class Preview {
	...
	static buildPreviewSettings(urlParams?: Partial<PreviewSettings>): PreviewSettings {
		const allowedPreviewSettings: PreviewSettings = fromEntries(
			previewSettingsKeys.map((k) => [k, undefined]),
		);
	...
	static deserialize(jsonPreview: PreviewSerialized) {
		const items: CartItem[] = jsonPreview.items.map(
			(item) => new CartItem(item.id, item.dataobject, item.type, item.url),
		);
	...
	initPreview(
		...
		const objects = ids.map((id) => {
			...
			const previewInfo: PreviewInfo | undefined =
				objInfo ? lookup.forDataObjSpec(objInfo.spec)
				: cart.hasItem(id) ? lookup.forDataObjSpec(cart.item(id)!.spec)
				: undefined;

			const options: OptionWithType =
				previewInfo == undefined ? { type: undefined, options: [] }
				: previewInfo.type === "TIMESERIES" ? previewInfo
				: { type: previewInfo.type, options: [] };

			const item = objInfo ? new CartItem(id, objInfo, options.type) : undefined;
			return { options, item };
		});
		...
		if (items.length) {
			if (options.type === "TIMESERIES") {
				...
				const xAxis = config.previewXaxisCols.find((x) =>
					options.options.some((op) => op.varTitle === x),
				);
				if (xAxis) {
				...
				options.type === config.NETCDF
				|| options.type === config.MAPGRAPH
				|| options.type === config.PHENOCAM
			) {
	...
	restore(lookup: PreviewLookup, cart: Cart, objectsTable: KnownDataObject[]) {
		if (this.hasPids) {
			return this.initPreview(
				lookup,
				cart,
				this.pids.map((pid) => config.objectUriPrefix[config.envri] + pid),
				objectsTable,
			);
		} else {
			return this;
	...
	withItemUrl(url: UrlStr) {
		return new Preview(
			this.items.map((i) => i.withUrl(url)),
			this.options,
			this.type,
		);
	}
...
export function previewAvailability(
	...
	obj: { spec: string; dobj: string; submTime: Date },
): PreviewAvailability {
	...
	if (
		previewType === "NETCDF"
		&& obj.spec !== config.netCdf.cfSpec
		&& !lookup.hasVarInfo(obj.dobj)
	)
