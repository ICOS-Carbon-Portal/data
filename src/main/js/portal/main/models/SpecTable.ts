...
export type Filter = null | Value[];
export type Col<T extends string> = T | typeof SPECCOL;
export type Filters<T extends string> = Partial<Record<T, Filter>>;
export type Row<T extends string> = Record<Col<T>, Value>;
...
export const Filter = {
	...
	allowsNothing(f: Filter): boolean {
...
export const Value = {
	isDefined(v: Value): v is (number | string) {
		return v !== undefined;
	},
	isString(v: Value): v is string {
...
export default class SpecTable<T extends string = string> {
	...
	constructor(readonly colNames: T[], readonly rows: Array<Row<T>>, readonly filters: Filters<T>, readonly extraSpecFilter: Filter = null) {
	...
	withFilter(colName: T, filter: Filter): SpecTable<T> {
		...
		const newFilters = {...this.filters, [colName]: filter};
