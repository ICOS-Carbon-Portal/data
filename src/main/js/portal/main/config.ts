import {
	type BaseMapId, type BaseMapFilter, cirlcePointStyle, supportedSRIDsFriendlyNames, type SupportedSRIDs
} from "icos-cp-ol";
import type Style from "ol/style/Style";
import commonConfig, {
	envri as untypedEnvri, type ICOS, type SITES, type NETCDF, type TIMESERIES, type MAPGRAPH, type PHENOCAM, type ICOSCities
} from "../../common/main/config";
import {type UrlStr} from "./backend/declarations";
import {type IndexedDBProps} from "./backend/IndexedDB";
...
type EnvriUrl = Record<Envri, UrlStr>;
...
const additionalStationsGraphs: Partial<Record<Envri, UrlStr[]>> = {
...
const sridsInMap: Record<Envri, Record<SupportedSRIDs, string>> = {
	ICOS: {
		3035: supportedSRIDsFriendlyNames["3035"],
		54_030: supportedSRIDsFriendlyNames["54030"]
	},
	SITES: {
...
const defaultSRID: Record<Envri, SupportedSRIDs> = {
	ICOS: "3035",
	SITES: "3006",
	ICOSCities: "3035"
};

const defaultBaseMap: Record<Envri, BaseMapId> = {
	ICOS: "physical",
	SITES: "lmTopoGray",
	ICOSCities: "physical",
};

const baseMapFilter: Record<Envri, BaseMapFilter> = {
...
export const numericFilterLabels: Record<NumberFilterCategories, string> = {
	samplingHeight: "Sampling height (meters)",
	fileSize: "File size (bytes)"
};

export const placeholders: Record<Envri, CategoryNamesDict> = {
	ICOS: defaultCategNames,
	SITES: {
		...defaultCategNames, station: "Station", ecosystem: "Ecosystem", project: "Thematic programme", valType: "Parameter"
	},
	ICOSCities: defaultCategNames
};

export type CategPrefix = UrlStr | Array<{prefix: string, value: UrlStr}>;
type PrefixConfig = Partial<Record<CategoryType, CategPrefix>>;

export const prefixes: Record<Envri, PrefixConfig> = {
	ICOS: {
...
type IFilterCategories = Record<Envri, ReadonlyArray<{
	panelTitle: string
...
export type PublicQueryDeclaration = {
	label: string
	comment: string
};
...
export const breadcrumbs: Record<Envri, readonly Breadcrumb[]> = {
	ICOS: [
