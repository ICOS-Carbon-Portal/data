...
import {type State, type TabsState, type WhoAmI} from "../models/State";
...
import {
	...
	searchDobjByFileName, sparqlFetchBlob
} from "../backend";
import {type ColNames, type OriginsColNames} from "../models/CompositeSpecTable";
import {type Sha256Str, type UrlStr} from "../backend/declarations";
import {
	FiltersNumber, FiltersUpdatePids, FiltersUpdateFileName, UiInactivateAllHelp, FiltersTemporal
} from "../reducers/actionpayloads";
import type FilterTemporal from "../models/FilterTemporal";
import {type HelpStorageListEntry, HelpItem, type HelpItemName} from "../models/HelpStorage";
...
import {type QueryParameters, type SearchOption} from "./types";
...
function getDobjOriginsAndCounts(fetchObjListWhenDone: boolean): PortalThunkAction<void> {
	return (dispatch, getState) => {
		...
		fetchDobjOriginsAndCounts(filters).catch(failWithError(dispatch)).then(dobjOriginsAndCounts => {
...
export const getFilteredDataObjects: PortalThunkAction<void> = (dispatch, getState) => {
	...
	scopedKeywords.fetch(options).catch(failWithError(dispatch)).then(scopedKeywords => {
		dispatch(new Payloads.BackendKeywordsFetched(scopedKeywords));
	});

	dataObjectsFetcher.fetch(options).catch(failWithError(dispatch)).then(({rows, isDataEndReached}) => {
...
const getOptions = (state: State, customPaging?: Paging): QueryParameters => {
	...
	return useOnlyPidFilter
		? pidFilterQparams
		: Object.assign(pidFilterQparams, {
			specs: Filter.and([
...
export const makeQuerySubmittable = (orgQuery: string) => {
	...
	return queryRows
		.filter(row => row.length > 0 && !/^\s*#/.test(row) && !/^\t+$/.test(row))
		.join("\n");
};
...
export function getAllFilteredDataObjects(): PortalThunkAction<void> {
	return (dispatch, getState) => {
		...
		sparqlFetchBlob(query).then(blob => {
			...
			lnk.href = globalThis.URL.createObjectURL(blob);
...
function fetchExtendedDataObjInfo(dobjs: UrlStr[]): PortalThunkAction<void> {
	return dispatch => {
		getExtendedDataObjInfo(dobjs).catch(failWithError(dispatch)).then(extendedDobjInfo => {
...
const logPortalUsage = (state: State) => {
	...
	if (categNames.length > 0 || filterTemporal.hasFilter || filterNumbers.hasFilters || filterKeywords.length > 0 || effectiveFilterPids !== null) {
		...
		if (filterTemporal.hasFilter) {
			filters.filterTemporal = filterTemporal.serialize;
		}

		if (filterNumbers.hasFilters) {
			filters.filterNumbers = filterNumbers.serialize;
		}

		if (effectiveFilterPids !== null) {
			filters.filterPids = effectiveFilterPids;
		}

		if (filterKeywords.length > 0) {
			filters.filterKeywords = filterKeywords;
		}
...
export function specFilterUpdate(varName: ColNames | "keywordFilter", values: Value[]): PortalThunkAction<void> {
	return dispatch => {
		...
		if (varNameAffectingCategs.includes(varName as ColNames)) {
...
export const filtersReset: PortalThunkAction<void> = (dispatch, getState) => {
	...
	const shouldRefetchCounts = filterTemporal.hasFilter
		...
		|| (Boolean(mapProps.rects) && mapProps.rects.length > 0);
...
export function setKeywordFilter(filterKeywords: string[], reset = false): PortalThunkAction<void> {
	return dispatch => {
		if (reset) {
			dispatch(new Payloads.MiscResetFilters());
		}
...
export function getFilterHelpInfo(name: ColNames | HelpItemName): PortalThunkAction<void> {
	return (dispatch, getState) => {
		...
		if (helpItem === undefined) {
			...
			if (uriList.length > 0) {
				fetchResourceHelpInfo(uriList).catch(failWithError(dispatch)).then(resourceInfoRaw => {
...
export function updateSearchOption(searchOption: SearchOption): PortalThunkAction<void> {
	return (dispatch, getState) => {
		...
		if (mustFetchPIDs) {
			...
			dispatch(getDobjOriginsAndCounts(mustFetchObjs));
		} else if (mustFetchObjs) {
			dispatch(getFilteredDataObjects);
		}
	};
}
