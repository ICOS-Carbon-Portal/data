...
import type P from "../../models/Paging";
import config from "../../config";
import {type ExportQuery, type SearchOptions} from "../../models/State";
...
export const Paging = (props: Paging) => {
	...
	const to = Math.max(minObjs, pageCount);
		...
		return (
			<div className="card-footer">
				<div style={{textAlign: "right", lineHeight: "1rem"}}>
					<StepButton direction="step-backward" enabled={offset > 0} onStep={() => {
						window.scrollTo(0, 0); requestStep(-1);
					}} />
					<StepButton direction="step-forward" enabled={isForwardEnabled} onStep={() => {
						window.scrollTo(0, 0); requestStep(1);
					}} />
				</div>
			</div>
