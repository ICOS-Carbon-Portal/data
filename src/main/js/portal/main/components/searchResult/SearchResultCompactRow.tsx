...
import {
	formatBytes, formatDateWithOptionalTime, pick, getUrlWithEnvironmentPrefix
} from "../../utils";
import {type ExtendedDobjInfo, type KnownDataObject, type WhoAmI} from "../../models/State";
...
type Props = {
	...
	preview: Preview
	extendedDobjInfo?: Array<ExtendedDobjInfo | undefined>
...
export default class SearchResultCompactRow extends Component<Props> {
	...
	render() {
		...
		const className = previewItem && previewItem.dobj === objInfo.dobj
			...
			: "";
		const size = Number.parseInt(objInfo.size);
...
const CollectionLinks: React.FunctionComponent<{extendedDobjInfo?: Array<ExtendedDobjInfo | undefined>, dobj: string}> = ({extendedDobjInfo, dobj}) => {
	if (extendedDobjInfo?.[0] === undefined) {
		return null;
	}
