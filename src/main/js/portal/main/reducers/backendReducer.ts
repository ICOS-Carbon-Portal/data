import stateUtils, {type CategFilters, type KnownDataObject, type State} from "../models/State";
import config, {type CategoryType} from "../config";
import CompositeSpecTable, {type ColNames} from "../models/CompositeSpecTable";
...
const handleOriginsTable = (state: State, payload: BackendOriginsTable) => {
	...
	if (isInPidFilteringMode(state.tabs, state.filterPids)) {
		return {specTable};
	}
