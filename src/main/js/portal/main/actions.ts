...
import stateUtils, {Profile, State, SearchOptions, WhoAmI, ObjectsTable, StateSerialized, Route} from "./models/State";
import {
	getCart,
	saveCart,
	logOut,
...
export function updateRoute(route: string): PortalThunkAction<void> {
	return (dispatch) => {
		...
		dispatch(new Payloads.UiUpdateRoute(route as Route));
